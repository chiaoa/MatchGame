...
function shuffleCardImageClasses() {
  ...
  let image = ["image-1", "image-1", "image-2", "image-2", "image-3", "image-3", "image-4", "image-4", "image-5", "image-5", "image-6", "image-6"];
   ...
   */
  shuffledImageClasses = _.shuffle(image);
...
function createCards(parentElement, shuffledImageClasses) {
  ...
  // Step 2: Loop 12 times to create the 12 cards we need.
    ...
    // Step 2(b): Add an image class to the new card element, using shuffledImageClasses[i].
      ...
      imgClass = shuffledImageClasses[i];
...
function onCardFlipped(newlyFlippedCard) {
  ...
  console.log(newlyFlippedCard.imageClass);
  console.log(lastCardFlipped.imageClass);

  match = doCardsMatch(lastCardFlipped, newlyFlippedCard);
  console.log(match);
  if (match == false) {
  ...
  matchCount = document.getElementById("match-count").innerHTML;
  // if (matchCount == 6) {
  //   winAudio.play();
